...
ref = ref_uni.select_atoms(sel_fit)  # for overall fitting

ref_selections = {}  # for each calc elements reference
for k, v in sel_calc.items():
    sel = ref_uni.select_atoms(selection_tmpl % v)
    ref_selections[k] = sel

# 2.2. mobile
mob = uni.select_atoms(sel_fit)  # for overall fitting
...
z = np.zeros(t_tot)
results = {}
for k in sel_calc.keys():
    results[k] = z.copy()


# 4. reference coordinate sets
ref_coord = ref.coordinates()
...
for ts in uni.trajectory:
    ...
    mob_coord = mob.coordinates()
    com_mob = centroid(mob_coord)
    mob_coord -= com_mob

    rot_matrix = kabsch(mob_coord, ref_coord)

    for k, sel_calc in selections.items():
        cal_coord = sel_calc.coordinates()
        cal_coord -= com_mob
...
for k, v in results.items():
    sbn.tsplot(v, time=t)
    sbn.plt.title('RMSD of %s; fitting %s' % (k, sel_fit))
