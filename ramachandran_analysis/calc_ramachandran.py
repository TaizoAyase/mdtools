...
def calc_dihedral(v1, v2, v3):
    ...
    # calc angle 
    cos_th = np.sum(ax1 * ax2, axis=1) / np.linalg.norm(ax1, axis=1) / np.linalg.norm(ax2, axis=1)
...
def calc(uni):
    ...
    sel_C  = uni.selectAtoms(*["resid %d and name C"  % x for x in xrange(res_start, res_end + 1)])
    sel_N  = uni.selectAtoms(*["resid %d and name N"  % x for x in xrange(res_start, res_end + 1)])
    sel_CA = uni.selectAtoms(*["resid %d and name CA" % x for x in xrange(res_start, res_end + 1)])
    ...
    for ts in uni.trajectory:
        sys.stderr.write("Processing frame %d/%d...\r" % (ts.frame, len(uni.trajectory)))
